...
class _LoginViewState extends State<LoginView> {
  ...
  Future<void> _generateQrCode() async {
    ...
    final result = await TvBilibiliFacade.generateTvQrCode();
    if (!mounted) return;
  ...
  void _startPolling() {
    ...
    _pollTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      ...
      final result = await TvBilibiliFacade.pollTvLogin(_authCode!);
      ...
      switch (status) {
        case 'success':
          timer.cancel();
          // 保存登录凭证
          await TvAccountFacade.saveLoginCredentials(
          ...
          // 获取用户信息
          await TvBilibiliFacade.fetchAndSaveUserInfo();
          if (!mounted) return;
