...
class LiveTabState extends State<LiveTab> {
  ...
  void _initCategories() {
    ...
    final liveOrder = TvSettingsFacade.liveCategoryOrder;
    for (var key in liveOrder) {
      if (TvSettingsFacade.isLiveCategoryEnabled(key)) {
        final label = TvSettingsFacade.liveCategoryLabels[key] ?? key;
        final id = TvSettingsFacade.liveCategoryIds[key];
        if (id != null) {
          _categories.add(
            _LiveCategory(
              id: key,
    ...
    try {
      ...
      if (category.type == _LiveCategoryType.following) {
        if (TvAccountFacade.isLoggedIn) {
          ...
          rooms = await TvLiveFacade.getFollowedLive(
            page: refresh ? 1 : currentPage,
            pageSize: 20,
          );
        }
      } else if (category.type == _LiveCategoryType.recommend) {
        rooms = await TvLiveFacade.getRecommended(
          page: refresh ? 1 : currentPage,
          pageSize: 30,
        );
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => LivePlayerScreen(
          ...
          uname: room['uname'],
          face: room['face'],
          online: TvLiveValueFacade.toInt(room['online'] ?? room['text_small']),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final showLoginText = isFollowingTab && !TvAccountFacade.isLoggedIn;
