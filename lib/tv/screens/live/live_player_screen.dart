...
class _LivePlayerScreenState extends State<LivePlayerScreen>
  ...
  final TvLiveSocketFacade _socketService = TvLiveSocketFacade();
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      if (TvSettingsFacade.hideLiveControlsOnStart) {
        _showControls = false;
      }
    });
  }
  ...
  Future<void> _initializePlayer({int? qn, int? lineIndex}) async {
    ...
    try {
      ...
      if (_realRoomId == widget.roomId) {
        roomInfo = await TvLiveFacade.getRoomInfo(widget.roomId);
      ...
      // 1. 获取播放地址
      final playInfo = await TvLiveFacade.getPlayUrl(
        _realRoomId,
      ...
      if (_anchorUid > 0) {
        TvLiveFacade.getRelation(_anchorUid).then((relation) {
  ...
  void _handleControlSelect() async {
    ...
    if (_focusedControlIndex == 0) {
      ...
      final success = await TvLiveFacade.modifyRelation(_anchorUid, act);
      if (success) {
        setState(() {
