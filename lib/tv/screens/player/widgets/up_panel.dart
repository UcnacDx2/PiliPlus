...
class UpPanel extends StatefulWidget {
  ...
  final String upFace;
  final int upMid;
  final Function(TvVideoItem) onVideoSelect;
...
class _UpPanelState extends State<UpPanel> {
  ...
  Future<void> _loadData() async {
    ...
    final results = await Future.wait([
      TvBilibiliFacade.getSpaceVideos(mid: widget.upMid, order: _order),
      TvBilibiliFacade.checkFollowStatus(widget.upMid),
    ]);

    if (mounted) {
      setState(() {
        _videos = results[0] as List<TvVideoItem>;
  ...
  Future<void> _toggleSort() async {
    ...
    final videos = await TvBilibiliFacade.getSpaceVideos(
      mid: widget.upMid,
      order: newOrder,
    );
    if (mounted) {
  ...
  Future<void> _toggleFollow() async {
    final success = await TvBilibiliFacade.followUser(
      mid: widget.upMid,
      follow: !_isFollowing,
    );
  ...
  Widget _buildVideoItem(TvVideoItem video, bool isFocused) {
    return Container(
      height: 88,
